...
export default function InvestmentChart() {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          "http://localhost:3000/api/investmentChartData"
        );
        ...
        setError(
          "error: Failed to load investment data. Please try again later."
        );
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div>
        <h3 className="text-lg font-bold mb-4">Yearly Total Investment</h3>
